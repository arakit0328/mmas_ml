...
SCPinstance::SCPinstance(FILE *SourceFile)
{
  ...
  else
  {
    ...
    for(int i = 0;  i < numRows; i++)
    {
      ...
      for(int j = 0; j < CoverNo; j++)
      {
        ...
        {
          Rows[i].Covers[j] = CoverID - 1; // 1引いた値を保持する
          Cols[CoverID - 1].numEntry++;
        }
        else
          throw (DataException());
      }
    ...
    for (int i = 0; i < numColumns; i++) Lindex[i] = 0;

    for (int i = 0; i < numRows; i++)
    {
      for (int j = 0; j < Rows[i].numCover; j++)
      {
  ...
  // 簡単な方法でデータの正しさを確認
  long Sum1 = 0, Sum2 = 0;
  for (int i = 0; i < numRows; i++) Sum1 += Rows[i].numCover;
  for (int j = 0; j < numColumns; j++) Sum2 += Cols[j].numEntry;
...
SCPsolution::SCPsolution(SCPinstance &pData, int k)
{
  ...
  for (int i = 0; i < nCol; ++i) SOLUTION.push_back(0);
  for (int i = 0; i < nCol; ++i) SCORE.push_back(pData.Cols[i].numEntry);
  for (int i = 0; i < nRow; ++i) COVERED.push_back(0);
  for (int i = 0; i < nRow; ++i) INACTIVE_LEVEL.push_back(1);
}
...
void SCPsolution::add_column(SCPinstance &pData, int c)
{
  ...
  int j = 0;
  while (CS[j] < c && j < K) j++;
  if (j == K) { printf("Cannot add column %d\n", c);  exit(1); }
  for (int jj = K-1; jj >= j; jj--) CS[jj + 1] = CS[jj];
  CS[j] = c;
  CS[K] = nCol + 1;

  // スコア更新
  for (int i = 0; i < pData.Cols[c].numEntry; i++)
  {
    ...
    if (COVERED[r] == 1)
    {
      ...
      for (int ri = 0; ri < pData.Rows[r].numCover; ri++)
      {
...
void SCPsolution::remove_column(SCPinstance &pData, int c)
{
  ...
  // 削除する要素をCSの末尾に移動して，最後の要素には大きな値を入れる
  int j = 0;
  while (CS[j] != c) j++;
  std::swap(CS[j], CS[K-1]);
  CS[K-1] = nCol + 1;
...
// DoubleLayerSelection
// 未完成 あとでやる
// independent row を処理する仕組みが必要
void SCPsolution::double_layer_selection(SCPinstance& instance,
                                         Rand& rnd)
{
  initialize(instance);

  std::vector<int> maxCols;
  int maxScore = 0, col = 0;
  ...
  for (int k = 0; k < K; ++k)
  {
    ...
    {
      // 本当は independent row を含む列を選ぶが，今回はとりあえずランダムにする
      int j = rnd(0, maxCols.size() - 1);
      col = maxCols[j];
    }

    add_column(instance, col);
  } // End while k < K
